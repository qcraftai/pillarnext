    ...
    def __init__(
        ...
        **kwargs,
        ...
        if stride > 1:
            ...
            self.deblock = ConvBlock(in_channels, head_conv, kernel_size=int(stride), stride=int(stride), conv_cfg=deconv_cfg)
...
class CenterHead(nn.Module):
    def __init__(
        ...
        for (num_cls, stride) in zip(num_classes, strides):
            ...
            self.tasks.append(
                SepHead(share_conv_channel, heads, stride = stride, bn=True, init_bias=init_bias, final_kernel=3)
            )
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.with_iou or self.with_reg_iou:
                ...
                xs = xs * self.out_size_factor[task_id] * self.voxel_size[0] + self.pc_range[0]
                ys = ys * self.out_size_factor[task_id] * self.voxel_size[1] + self.pc_range[1]
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            xs = xs * test_cfg.out_size_factor[task_id] * test_cfg.voxel_size[0] + test_cfg.pc_range[0]
            ys = ys * test_cfg.out_size_factor[task_id] * test_cfg.voxel_size[1] + test_cfg.pc_range[1]
            ...
            rets.append(self.post_processing(task_id, batch_box_preds, batch_hm, test_cfg, post_center_range, batch_iou)) 

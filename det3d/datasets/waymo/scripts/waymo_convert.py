...
def extract_points_from_range_image(laser, calibration, frame_pose):
    ...
    for range_image_str in [first_return, second_return]:
        ...
        points_tensor = points_tensor[:, :6] # x, y, z, intensity, elongation, nlz (1 for nlz)
...
def extract_objects(laser_labels, pose):
    ...
    for object_id, label in enumerate(laser_labels):
        ...
        speed = np.array([label.metadata.speed_x, label.metadata.speed_y]).reshape([2, 1])
...
def convert(idx):
    ...
    for frame_id, data in enumerate(dataset):
        ...
        lidar_path = 'lidar_point/' + token+'.bin'
        save_root = '/mnt/sda/jinyu/datasets/waymo/'
        points.tofile(save_root + lidar_path)
        ...
        for ii in range(1, 5):
            ...
            sw['timestamp'] = (frame.timestamp_micros - int(sw['token'].split('-')[1])) / 1e6
            sweeps.append(sw)
...
if __name__ == '__main__':
    root = '/media/data/waymo/perception/v1.4.0/tfrecord/'
    save_root = '/mnt/sda/jinyu/datasets/waymo_det/'
    for split in ['train','val']: 
        scenes = list(glob.glob(os.path.join(root, split, '*.tfrecord')))
        with Pool(32) as p:
            r = list(tqdm(p.imap(convert, range(len(scenes))), total=len(scenes)))
            
        all_infos = list(itertools.chain.from_iterable(r))
        with open(save_root + 'waymo_infos_' + split + '.pkl', 'wb') as f:

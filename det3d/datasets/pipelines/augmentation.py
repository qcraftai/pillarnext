        ...
        if self.flip_prob[0] > 0:
            ...
            if enable:
                ...
                if 'annotations' in res and 'gt_boxes' in res['annotations']:
                    ...
                    res['annotations']['gt_boxes'] = box_np_ops.flip(
                        res['annotations']['gt_boxes'], axis='x')
        ...
        if self.flip_prob[1] > 0:
            ...
            if enable:
                ...
                if 'annotations' in res and 'gt_boxes' in res['annotations']:
                    ...
                    res['annotations']['gt_boxes'][mask] = 0
                    res['annotations']['gt_boxes'] = box_np_ops.flip(
                        res['annotations']['gt_boxes'], axis='y')
                    res['annotations']['gt_boxes'][mask] = np.nan
        return res
...
class Scaling(object):
    ...
    def __call__(self, res):
        ...
        if 'annotations' in res and 'gt_boxes' in res['annotations']:
            mask = np.isnan(res['annotations']['gt_boxes'])
            res['annotations']['gt_boxes'][mask] = 0
            res['annotations']['gt_boxes'] = box_np_ops.scaling(
                res['annotations']['gt_boxes'], noise_scale)
            res['annotations']['gt_boxes'][mask] = np.nan
        return res
...
class Rotation(object):
    ...
    def __call__(self, res):
        ...
        res['points'][:, :3] = box_np_ops.yaw_rotation(
            res['points'][:, :3], noise_rotation)

        if 'annotations' in res and 'gt_boxes' in res['annotations']:
            mask = np.isnan(res['annotations']['gt_boxes'])
            res['annotations']['gt_boxes'][mask] = 0
            res['annotations']['gt_boxes'] = box_np_ops.rotate(
                res['annotations']['gt_boxes'], noise_rotation)
            res['annotations']['gt_boxes'][mask] = np.nan
        return res
...
class Translation(object):
    ...
    def __call__(self, res):
        ...
        if 'annotations' in res and 'gt_boxes' in res['annotations']:
            mask = np.isnan(res['annotations']['gt_boxes'])
            res['annotations']['gt_boxes'][mask] = 0
            res['annotations']['gt_boxes'] = box_np_ops.translate(
                res['annotations']['gt_boxes'], noise_translate)
            res['annotations']['gt_boxes'][mask] = np.nan
        return res

...
class AssignLabel(object):
    ...
    def __call__(self, res):
        ...
        for task_id, task in enumerate(self.tasks):
            ...
            hm = np.zeros((len(task), feature_map_size[1], feature_map_size[0]), dtype=np.float32)
        ...
        for k in range(len(gt_dict['gt_names'])):
            ...
            size_x = size_x / self.voxel_size[0] / self.out_size_factor[task_id]
            size_y = size_y / self.voxel_size[1] / self.out_size_factor[task_id]
            ...
            if size_x > 0 and size_y > 0:
                ...
                coor_x, coor_y = (x - self.pc_range[0]) / self.voxel_size[0] / self.out_size_factor[task_id], \
                                (y - self.pc_range[1]) / self.voxel_size[1] / self.out_size_factor[task_id]
                ...
                gt_boxes[task_id][new_idx] = np.concatenate(
                    (gt_dict['gt_boxes'][k][0:6], gt_dict['gt_boxes'][k][8]), axis=None
                )

                task_nums[task_id] += 1
        
        res.update({'hm': hms, 'anno_box': annos, 'ind': inds, 'mask': masks, 'cat': cats, 'gt_boxes': gt_boxes})                  
